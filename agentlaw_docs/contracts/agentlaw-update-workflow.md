...
Governing law: `agentlaw_docs/law/REPOSITORY_ARTIFACT_RULES.md`. Amendments land
...
- runs the pip package's startup hook to **automatically apply any pending schema migrations** to `.agentlaw/index/meta.db`. Binary DB ALTER cannot be done safely by the LLM, so the pip package owns it.
