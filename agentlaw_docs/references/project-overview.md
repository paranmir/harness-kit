...
- **AI coding agents**: read the law layer (`agentlaw_docs/law/*`) and this file
...
| `agentlaw_docs/law/*` | law layer (canonical rules) |
| `agentlaw_docs/contracts/*` | shared boundary contracts |
| `agentlaw_docs/planning-protocol/*` | plan template + review method |
| `agentlaw_docs/plans/{draft,active,completed}/` | plans by lifecycle stage |
| `agentlaw_memory/` | canonical memory (rules, known-facts, logs, working-set) |
| `.agentlaw/` | derived runtime state (gitignored — recreated by `agentlaw init`) |
