...
`agentlaw_docs/planning-protocol/*` so planning rules stay usable without bloating
`agentlaw_docs/law/*`.
...
`agentlaw_docs/planning-protocol/task-classification.md` § Non-Trivial Triggers. Trivial
...
3. Determine importance/risk by checking against non-trivial triggers (per
   `agentlaw_docs/planning-protocol/task-classification.md` § Non-Trivial Triggers).
4. Draft the plan using the structure in
   `agentlaw_docs/planning-protocol/plan-template.md`. Cite each matched trigger in the
...
6. For non-trivial plans:
   ...
   b. Run the Trigger Coverage Verifier first in an isolated review pass; it validates
      ...
      plan content.
   c. Run the Deep Review Selector per `agentlaw_docs/planning-protocol/review-method.md`.
   ...
   g. Revise the plan once. When revisions affect a specific subset of plan
      ...
      sections (per `agentlaw_docs/planning-protocol/persona-section-map.md`) include
...
live under `agentlaw_docs/plans/draft/` until they pass through the loop.
...
- `agentlaw_docs/planning-protocol/task-classification.md` for deciding whether
  planning is required, identifying task class(es), and determining trivial
  vs non-trivial via structural triggers
- `agentlaw_docs/planning-protocol/review-method.md` for the plan review and
  consolidation procedure, including Deep Review Selection
- `agentlaw_docs/planning-protocol/persona-decks-core.md` for the universal review
  bench
- `agentlaw_docs/planning-protocol/persona-decks-specialized.md` for substance- and
  class-specific review personas
- `agentlaw_docs/planning-protocol/plan-template.md` for the plan document structure
  that non-trivial plans must follow
- `agentlaw_docs/planning-protocol/persona-section-map.md` for the persona ↔ plan-
...
- `classification_source`: `agentlaw_docs/planning-protocol/task-classification.md`
- `review_method_source`: `agentlaw_docs/planning-protocol/review-method.md`
- `persona_deck_sources`: `agentlaw_docs/planning-protocol/persona-decks-core.md` and
  `agentlaw_docs/planning-protocol/persona-decks-specialized.md`
- `plan_template_source`: `agentlaw_docs/planning-protocol/plan-template.md`
- `persona_section_map_source`: `agentlaw_docs/planning-protocol/persona-section-map.md`
...
- `execution_gates_source`: `agentlaw_docs/planning-protocol/plan-template.md`
- `review_coverage_matrix_source`: `agentlaw_docs/planning-protocol/plan-template.md`
...
artifact under `agentlaw_docs/planning-protocol/` that defines the rule system's
...
the move into `agentlaw_docs/plans/completed/` is permitted. The archive tool also
...
`agentlaw_docs/planning-protocol/substance-deck-*.md`. The deck declares the
