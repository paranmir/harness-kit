...
- `agentlaw_docs/references/*` is treated as the active output target
...
Examples:
- memory summary overrides `agentlaw_docs/law/*`
...
`agentlaw_docs/law/USER_INTENT_ALIGNMENT.md` introduces a behavior-level
...
`agentlaw_docs/plans/tech-debt-tracker.md`, record both the behavior tag(s)
