...
- **The plan body itself**, when the work is plan-required per
  `agentlaw_docs/planning-protocol/task-classification.md` §Non-Trivial
...
- **A lightweight plan**, per
  `agentlaw_docs/planning-protocol/lightweight-plan-template.md`, when the
...
classification under `agentlaw_docs/planning-protocol/task-classification.md`:
...
taxonomy in `agentlaw_docs/law/FAILURE_TAXONOMY.md` is recorded in that
...
`agentlaw_docs/law/FAILURE_TAXONOMY.md` connects each tag to the broader
