...
- Code Plan Fidelity Profile checks that apply only to active plans marking `- substance: code`. Active-plan preflight fails when the plan lacks repository discovery evidence, a behavior contract, a change matrix with test/check pairing, focused verification, stop conditions, or runnable-oracle/verification-trace evidence for code behavior criteria. Completed legacy plans are not retroactively failed by this profile.
