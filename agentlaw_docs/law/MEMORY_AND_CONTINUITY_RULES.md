...
- canonical memory files under `agentlaw_memory/*`
- exact lookup, FTS, and vector indexes co-located in `.agentlaw/index/meta.db` (vector via the `sqlite-vec` extension; no separate vector directory)
- embedding model artifacts under `.agentlaw/models/embedding/<model-name>/`
- runtime job and cache paths under `.agentlaw/jobs/` and `.agentlaw/cache/`
...
Memory must not:
- override the constitution, root control documents, or `agentlaw_docs/law/*`
...
Derived runtime and index state belongs under `.agentlaw/`:
...
`agentlaw_memory/*` is the source layer. `.agentlaw/*` is derived runtime or index state and may be repaired from source material.
...
## Memory Types
`agentlaw_memory/known-facts/` stores currently remembered facts. Known facts are memory records, not governing rules. Each known fact must carry source, status, and verification metadata once the format is defined.

`agentlaw_memory/logs/` stores historical events. Logs are append-oriented. Incorrect past conclusions should be corrected by later log entries rather than erased by default.

`agentlaw_memory/rules/` stores project-local governance rules. Rules are distinct from known facts: facts are remembered state, rules are local law-level policy for the repository's own authoring workflow. Rules sit below shared `agentlaw_docs/law/*` in authority and above known facts and preferences. Full framework in the "Local Rule Memory Type" section below.

`agentlaw_memory/preferences.md` stores user or maintainer preferences. Preferences may guide style and workflow, but they lose to law, current explicit instruction, and safety or verification requirements.

`agentlaw_memory/working-set.md` stores current active context for low-token session continuity. It must be refreshed when active plans, current decisions, or next actions change. The working-set file is rewritten in full on every `agentlaw_session_save`; bullets inside the file are not separately addressable as item-level records and are not declared as a memory item type.
...
`agentlaw_memory/LOOKUP_RULES.md` stores retrieval policy. It tells agents and tools how to choose memory sources; it does not create higher authority than the memory layer.
...
### Authority Position
Rules sit strictly below shared `agentlaw_docs/law/*` and root control documents, and strictly above known facts, preferences, logs, and working-set entries. Conflicts resolve as follows:
...
### Front Matter Schema
Every rule file under `agentlaw_memory/rules/` must carry the following front matter:
...
### File Layout
One rule per file under `agentlaw_memory/rules/`. File names are lowercase kebab-case and describe the rule's scope or subject:

```text
agentlaw_memory/rules/release-readiness-gate.md
agentlaw_memory/rules/dependency-pinning-policy.md
```
...
- **Lookup read order**: `agentlaw_memory/LOOKUP_RULES.md` must consult rules before known facts when both are relevant. Active rules with `applies_when: [always]` are always in-scope for the current session.
...
### Known Fact File
Known facts should use one fact per file under `agentlaw_memory/known-facts/`.

File names should be lowercase kebab case and describe the remembered fact:

```text
agentlaw_memory/known-facts/current-active-plan.md
agentlaw_memory/known-facts/test-command.md
```
...
scope: repository
source:
  - path: agentlaw_docs/plans/active/harness-default-memory-distribution-revision-plan.md
...
The current active memory distribution driver is `agentlaw_docs/plans/active/harness-default-memory-distribution-revision-plan.md`.
...
### Rule File
Rules use one rule per file under `agentlaw_memory/rules/`. File names are lowercase kebab-case and describe the rule's subject:
...
applies_when: [always]
source:
  - path: agentlaw_docs/plans/completed/<plan-that-introduced-this-rule>.md
    relation: derived_from
last_checked: YYYY-MM-DD
supersedes: []
tags: [release, gate]
...
### Log File
Logs should be date- or session-grouped under `agentlaw_memory/logs/`.

Recommended path:

```text
agentlaw_memory/logs/YYYY-MM/YYYY-MM-DD.md
```
...
### Preferences File
Preferences live in `agentlaw_memory/preferences.md`.
...
The `memory_save_item(type="preference", ...)` write path edits `agentlaw_memory/preferences.md` in place using section-aware editing. The grammar that the writer recognizes:
...
### Working Set File
The working set lives in `agentlaw_memory/working-set.md`.
...
## Active Plan
- `agentlaw_docs/plans/active/harness-default-memory-distribution-revision-plan.md`

## Current Decisions
- Memory is default-on.
- Canonical memory uses `agentlaw_memory/known-facts/`, `logs/`, `preferences.md`, `working-set.md`, and `LOOKUP_RULES.md`.
...
`agentlaw_memory/logs/YYYY-MM/YYYY-MM-DD.md` and indexed for retrieval.
...
current_goal: string
active_plans:
  - path: agentlaw_docs/plans/active/example.md
    status: active
last_decisions:
  - summary: string
...
next_actions:
  - action: string
    source: path-or-memory-id
relevant_sources:
  - path: agentlaw_docs/law/MEMORY_AND_CONTINUITY_RULES.md
...
Rules:
- `agentlaw_memory/working-set.md` is the primary current session state.
- `agentlaw_memory/LOOKUP_RULES.md` tells agents how to restore context without broad reading.
- `agentlaw_memory/logs/*` records session decisions and handoff-relevant events.
- Session restore must start from `agentlaw_memory/working-set.md` when it exists.
- If `agentlaw_memory/working-set.md` is missing or stale, mark the restore as degraded and recover context from logs and source files.
- Session save must update `agentlaw_memory/working-set.md` first when the file exists.
- Session save should append a short session entry to `agentlaw_memory/logs/*` when the session produced durable decisions.
...
2. Read `agentlaw_memory/working-set.md` in full.
3. Read the body of every law file in full — `AGENTLAW_CONSTITUTION.md` and every file under `agentlaw_docs/law/*`. The law layer is short and binding; rules that the session must apply must live in the agent's working context, not just as paths.
4. Read the body of every active plan — every file under `agentlaw_docs/plans/active/*`. Active plans are the work currently in flight; titles alone do not carry slices, contracts, or non-goals.
5. Read the body of the most recent `session_save` log entry — the single newest `agentlaw_memory/logs/YYYY-MM/YYYY-MM-DD.md` entry of `kind: session_save`. This is the directly preceding session's wrap-up summary, the most concentrated source of "what landed last" and "what is open right now".
6. List `agentlaw_memory/logs/*` entries dated within `log_lookback_days_default` (from `LOOKUP_RULES.md`, default 14 days) for the broader timeline. Titles only at this step — body reads for entries other than the most recent `session_save` (already covered by step 5) belong to the Conditional Tier.
7. Read the body of every entry in `active_rules`. Rule bodies live in `agentlaw_memory/rules/*` and must be read via the canonical memory source path; the packet's `active_rules` field carries only summary metadata, which is insufficient for applying the rule's full procedure or constraint.
8. Read `agentlaw_memory/preferences.md` in full. Preferences shape style and workflow choices the session should respect; the file is short, single-doc, and applies on every session.
9. Read `agentlaw_memory/LOOKUP_RULES.md` in full. The retrieval policy (read order, trigger map, budgets, exclusions, escalation) governs how the agent uses memory throughout the session and must live in the agent's working context, not be re-derived turn by turn.
10. Scan the known-facts manifest — collect the id, title, scope, and last_checked timestamp of every file under `agentlaw_memory/known-facts/*.md` whose status is `active`. **Bodies are not read at this step.** The manifest gives the agent a complete catalog of "what facts are remembered" so it does not invent or contradict known state; bodies are fetched on demand under step 11 or via working-frame search at step 11a.
...
12. Detect governance drift: list `agentlaw_docs/law/*` and `AGENTLAW_CONSTITUTION.md` files modified after the working set's `updated_at` timestamp. Flag any drift in `authority_warnings`.
...
15. Read a specific `agentlaw_memory/known-facts/*.md` entry's body only when the working-frame search at step 11 hit it, when `working-set.md` references it by id or path, or when the user explicitly asks for it. The manifest from step 10 plus the search hits from step 11 together drive this expansion.
...
### Degradation Rules
- If `agentlaw_memory/working-set.md` is missing or stale, mark the restore as `degraded` and recover context from logs and active plans.
...
5. Add or update `agentlaw_memory/known-facts/*.md` entries when the session produced new durable facts or superseded existing ones.
6. Update `agentlaw_memory/preferences.md` when the user expressed a stable, narrow preference that should survive across sessions.
7. Update `agentlaw_docs/plans/active/*` when the session changed plan content; move completed plans to `agentlaw_docs/plans/completed/`.
...
- Keep `agentlaw_memory/working-set.md` compact. Move history into logs instead of letting the working set become a changelog.
...
- `updated_at` in `agentlaw_memory/working-set.md` YAML front matter
- `occurred_at` and `recorded_at` in `agentlaw_memory/logs/*` entries
...
The LLM accesses indexed memory through the bundled MCP server only. Raw SQL against `.agentlaw/index/meta.db` and direct file reads of derived state are prohibited.

The full tool surface is exposed as default capabilities. The signature reference (parameters, return shapes, error semantics) lives at `agentlaw_docs/references/agentlaw-mcp-tools.md`. This document owns *when* and *why* to use each tool.
...
1. **Plan-review session pruning** (`agentlaw_plan_review_session_prune_proposal`
   ...
   `agentlaw_docs/plans/draft/active/completed/`.
