...
- `AGENTS.md`
- the current `agentlaw_docs/law/*` law documents
- `agentlaw_memory/working-set.md` for primary session context when present
- `agentlaw_memory/LOOKUP_RULES.md` for session lookup policy
...
`agentlaw_docs/law/PLANNING_AND_REVIEW_RULES.md` before execution. The operational
classification and persona-review details live in `agentlaw_docs/planning-protocol/*`.
...
- updated `agentlaw_memory/working-set.md` when session state changes
...
`agentlaw now --json` outputs one compact JSON object based on the host operating system clock. It is intended for timestamp integrity workflows, especially when an agent must manually write `agentlaw_memory/*` timestamps.
...
When a Harness repository uses the default memory structure, session restore should read `agentlaw_memory/working-set.md` first and use `agentlaw_memory/LOOKUP_RULES.md` for lookup decisions. Session save should update `agentlaw_memory/working-set.md` first and append a short `agentlaw_memory/logs/*` entry when durable decisions or handoff items were produced.
...
- `agentlaw_docs/plans/active/*`
- `agentlaw_docs/plans/completed/*`
- `agentlaw_docs/plans/tech-debt-tracker.md`
- `agentlaw_docs/references/*`
- `agentlaw_docs/planning-protocol/*` when reusable planning classification and
